...
export default function CardDisplay({ card, onClick, isSelected, canPlay = true, size = 'md' }: CardDisplayProps) {
  ...
  return (
    <>
      ...
      id={`card-${card.id}`}
      className={`relative flex flex-col ${dimensionsClass} border transition-all duration-300 cursor-pointer select-none overflow-hidden hover:scale-102 hover:-translate-y-1 ${factionBorderClass} ${
        isSelected ? 'ring-2 ring-amber-500/60 ring-offset-2 ring-offset-[#0a0a0c] scale-103 shadow-lg shadow-white/5' : ''
      } ${!canPlay ? 'opacity-40 cursor-not-allowed' : 'shadow-md shadow-black/80 bg-black/40'} ${
        instance?.isExhausted && hasImage && viewMode === 'image' ? 'grayscale opacity-75' : ''
      }`}
    >
      {/* Background radial highlight - subtle elegance */}
      <div className={`absolute -right-24 -top-24 w-44 h-44 rounded-full blur-3xl opacity-15 ${
        card.faction === 'Space Marines' ? 'bg-amber-500' : card.faction === 'Orks' ? 'bg-red-600' : 'bg-slate-400'
      }`} />
      ...
      {hasImage && viewMode === 'image' ? (
        ...
        <>
          ...
          <div className="flex justify-between items-start gap-1 pb-1.5 z-10">
            <h3 className={`font-heading font-semibold tracking-tight leading-snug line-clamp-2 ${
              size === 'mini' ? 'text-[8.5px]' : size === 'sm' ? 'text-[10px]' : size === 'lg' ? 'text-[15.5px]' : 'text-sm'
            ...
            {/* Cost Badge */}
            {card.type !== 'Warlord' ? (
              <div className={`flex items-center justify-center rounded-md bg-white/5 text-gray-300 border border-white/10 font-mono font-semibold ${
                size === 'mini' ? 'min-w-3.5 h-3.5 text-[8px] px-0.5' : size === 'sm' ? 'min-w-4 h-4 text-[9px] px-0.5' : size === 'lg' ? 'min-w-8 h-8 text-sm px-1' : 'min-w-6 h-6 text-xs px-1'
              }`}>
                {card.cost}
              </div>
            ) : (
              <div className={`flex items-center justify-center rounded bg-amber-500/10 text-amber-500 border border-amber-500/20 font-serif font-bold tracking-widest uppercase ${
                size === 'mini' ? 'text-[6px] px-0.5 py-0.1' : size === 'sm' ? 'text-[7px] px-0.5 py-0.2' : size === 'lg' ? 'text-[11px] px-1.5 h-7 flex items-center' : 'text-[10px] px-1 h-6'
              }`}>
                HQ
              </div>
            )}
          ...
          <div className={`flex items-center justify-between px-1.5 py-0.5 rounded border text-[8px] tracking-wider uppercase font-semibold font-heading z-10 ${
            size === 'mini' ? 'mb-0.5 text-[6.5px] py-0' : size === 'sm' ? 'mb-1 text-[7px] py-[1px]' : size === 'lg' ? 'mb-3.5 text-[10px] py-[2px]' : 'mb-3'
          } ${factionHeaderBg}`}>
          ...
          {card.type !== 'Support' && card.type !== 'Event' ? (
            <div className={`flex items-center justify-around bg-[#0a0a0c]/80 rounded-lg border border-white/5 font-mono z-10 ${
              size === 'mini' ? 'py-0.2 px-0.5 mb-0.5 text-[8px]' : size === 'sm' ? 'py-0.5 px-0.5 mb-1 text-[9px]' : size === 'lg' ? 'py-2 px-1 mb-3.5 text-sm' : 'py-1.5 px-1 mb-3 text-xs'
            }`}>
          ...
          {/* Card Rules text */}
          {size !== 'mini' && (
            <div className={`flex-grow text-gray-400 leading-normal z-10 font-sans italic p-1.5 bg-black/25 rounded border border-white/5 ${
              size === 'sm' ? 'line-clamp-2 text-[8px] tracking-tight p-0.5' : size === 'lg' ? 'text-[12px] line-clamp-5' : 'text-[9.5px] line-clamp-4'
            }`}>
              {card.description}
            </div>
          )}
          ...
          {hasImage && (
            <button
              ...
              className={`absolute p-1 rounded bg-black/60 hover:bg-black/85 border border-white/10 text-gray-300 hover:text-white transition-all z-35 ${
                size === 'mini' ? 'top-1 right-1 p-0.5' : size === 'sm' ? 'top-1 right-1 p-0.5' : size === 'lg' ? 'top-3 right-3 p-1' : 'top-2.5 right-2.5'
              }`}
            >
          ...
          {instance && (
            <div className={`flex justify-between items-center border-t border-white/5 z-10 ${
              size === 'mini' ? 'mt-0.5 pt-0.5' : size === 'sm' ? 'mt-1 pt-1' : size === 'lg' ? 'mt-4 pt-2.5' : 'mt-3 pt-2'
            }`}>
              <div className="flex items-center gap-1">
                {instance.isExhausted ? (
                  <span className={`bg-red-950/20 text-red-500 border border-red-900/20 px-1 py-0.2 rounded font-semibold uppercase tracking-wider ${
                    size === 'mini' ? 'text-[6.5px]' : size === 'sm' ? 'text-[7px]' : size === 'lg' ? 'text-[11px] px-1.5' : 'text-[9px]'
                  }`}>
                    EXHAUSTED
                  </span>
                ) : (
                  <span className={`bg-green-950/20 text-green-400 border border-green-900/20 px-1 py-0.2 rounded font-semibold uppercase tracking-wider ${
                    size === 'mini' ? 'text-[6.5px]' : size === 'sm' ? 'text-[7px]' : size === 'lg' ? 'text-[11px] px-1.5' : 'text-[9px]'
                  }`}>
                    READY
                  </span>
                ...
                {instance.isBloodied && size !== 'mini' && size !== 'sm' && (
                  <span className={`flex items-center bg-red-950/30 text-red-500 border border-red-500/20 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider ${
              ...
              {card.shields > 0 && (
                <div className={`flex items-center text-amber-500 font-mono ${
                  size === 'mini' ? 'text-[7px]' : size === 'sm' ? 'text-[8px]' : size === 'lg' ? 'text-[11px]' : 'text-[10px]'
                }`} title="Shield Count">
